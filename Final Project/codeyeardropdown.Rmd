...
```
```{r}
reg<-lm(Cost_of_living_Gap~year,data=acs_data2)
summary(reg)
```
```{r}
newdata<-data.frame(c(2023,2024,2025,2030))
names(newdata)<-"year"
...
#
mystates<-c('Alabama','Maryland','Arizona')
#NAME==|state
plot_data_full %>% filter(NAME%in%mystates) %>% 
  ggplot(aes(x=year, y=Cost_of_living_Gap, color=NAME,linetype=type))+
  geom_point()+
  geom_smooth(method="lm",se=FALSE)+
  labs(x="Year",y="Gap Between Cost of Living and Median Earnings",fill="State")+
...
#write_csv(acs_data, "Acsdata.csv")
#write_sf(acs_data,"acs_data.geojson")
acs_data2<-read_sf("acs_data2.geojson")
acs_data3<-acs_data2 %>% mutate(Cost_of_living_Gap=`Median Earnings`-`Cost of Living Index`)
